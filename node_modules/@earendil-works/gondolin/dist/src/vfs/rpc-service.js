...
exports.FsRpcService = exports.MAX_RPC_DATA = void 0;
...
class FsRpcService {
    constructor(provider, options = {}) {
        ...
        this.nextIno = 2;
        this.nextHandle = 1;
        this.pathToIno = new Map();
        this.inoToPath = new Map();
    ...
    async dispatch(op, req) {
        switch (op) {
            ...
            default:
                throw (0, errors_1.createErrnoError)(ERRNO.ENOSYS, op);
        }
    }
    async handleLookup(req) {
        ...
        const entryPath = normalizePath(path_1.default.posix.join(parentPath, name));
    ...
    async handleReaddir(req) {
        ...
        const entries = (await this.provider.readdir(entryPath, { withFileTypes: true }));
        ...
        for (let index = start; index < entries.length && responseEntries.length < maxEntries; index += 1) {
            ...
            const childPath = normalizePath(path_1.default.posix.join(entryPath, name));
    ...
    async handleRead(req) {
        ...
        if (size > exports.MAX_RPC_DATA) {
            throw (0, errors_1.createErrnoError)(ERRNO.EINVAL, "read");
        }
    ...
    async handleWrite(req) {
        ...
        if (data.length > exports.MAX_RPC_DATA) {
            throw (0, errors_1.createErrnoError)(ERRNO.EINVAL, "write");
        }
    ...
    async handleRename(req) {
        ...
        if (flags !== 0) {
            throw (0, errors_1.createErrnoError)(ERRNO.EINVAL, "rename");
        }
        ...
        const oldPath = normalizePath(path_1.default.posix.join(oldParentPath, oldName));
        const newPath = normalizePath(path_1.default.posix.join(newParentPath, newName));
        await this.provider.rename(oldPath, newPath);
        this.renameMapping(oldPath, newPath);
        return {};
    }
    async handleTruncate(req) {
    ...
    async handleRelease(req) {
        const fh = requireUint(req.fh, "release", "fh");
        const entry = this.handles.get(fh);
        if (!entry) {
            throw (0, errors_1.createErrnoError)(ERRNO.EBADF, "release");
        }
        this.handles.delete(fh);
        await entry.handle.close();
        return {};
    }
    async truncatePath(entryPath, size) {
    ...
    requirePath(ino, op) {
        ...
        if (!entryPath) {
            throw (0, errors_1.createErrnoError)(ERRNO.ENOENT, op);
        }
        return entryPath;
    }
    allocateHandle(handle, ino, entryPath, append) {
    ...
    getHandle(fh, op) {
        const entry = this.handles.get(fh);
        if (!entry) {
            throw (0, errors_1.createErrnoError)(ERRNO.EBADF, op);
        }
        return entry;
    }
    removeMapping(entryPath) {
        const normalized = normalizePath(entryPath);
...
exports.FsRpcService = FsRpcService;
function normalizePath(entryPath) {
    let normalized = path_1.default.posix.normalize(entryPath);
...
function validateName(name, op) {
    if (!name || name.includes("/") || name.includes("\0")) {
        throw (0, errors_1.createErrnoError)(ERRNO.EINVAL, op, name);
    }
}
function requireUint(value, op, field) {
    if (typeof value !== "number" || !Number.isFinite(value) || value < 0 || !Number.isInteger(value)) {
        throw (0, errors_1.createErrnoError)(ERRNO.EINVAL, op, field);
    }
    return value;
}
function requireString(value, op, field) {
    if (typeof value !== "string") {
        throw (0, errors_1.createErrnoError)(ERRNO.EINVAL, op, field);
    }
    return value;
}
function requireBuffer(value, op) {
    if (!Buffer.isBuffer(value)) {
        throw (0, errors_1.createErrnoError)(ERRNO.EINVAL, op);
    }
    return value;
}
function statsToAttr(ino, stats) {
    return {
        ino,
        mode: stats.mode,
        nlink: stats.nlink,
...
function openFlagsToString(flags, forceCreate) {
...
function parseOpenFlagsForOpen(flags) {
    const { O_RDONLY, O_WRONLY, O_RDWR, O_CREAT, O_TRUNC, O_APPEND } = LINUX_OPEN_FLAGS;
    if ((flags & O_CREAT) !== 0) {
...
async function direntType(entry, entryPath, provider) {
